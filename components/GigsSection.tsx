...
export default function GigsSection(): React.JSX.Element {
  ...
  const gigsPerPage = 6
  const upcomingGigs = allUpcomingGigs.slice(0, gigsPerPage + (upcomingGigsPage * gigsPerPage))
  const pastGigs = allPastGigs.slice(0, gigsPerPage + (pastGigsPage * gigsPerPage))
  
  const loadMoreGigs = useCallback(async (isUpcomingSection: boolean) => {
    if (loadingMore) return
    
    const currentPage = isUpcomingSection ? upcomingGigsPage : pastGigsPage
    const totalGigs = isUpcomingSection ? allUpcomingGigs.length : allPastGigs.length
  ...
  }, [upcomingGigsPage, pastGigsPage, loadingMore, allUpcomingGigs.length, allPastGigs.length, gigsPerPage])
  
  const handleScroll = useCallback((scrollContainer: HTMLDivElement | null, isUpcomingSection: boolean) => {
        ...
        {upcomingGigs.length > 0 ? (
                ...
                {loadingMore && (
                  <div className="w-72 sm:w-80 flex items-center justify-center flex-shrink-0">
                    <div className="glass rounded-xl p-6 text-center">
                      ...
                      <div className="text-sm text-text-muted">Loading more shows...</div>
                    </div>
                  </div>
                )}
                {hasMoreUpcoming && !loadingMore && allUpcomingGigs.length > upcomingGigs.length && (
                  <div className="w-72 sm:w-80 flex items-center justify-center flex-shrink-0">
                    <div className="glass rounded-xl p-6 text-center">
                      <div className="text-text-secondary mb-2">+{allUpcomingGigs.length - upcomingGigs.length} more shows</div>
                      <div className="text-sm text-text-muted">Keep scrolling to load</div>
                    </div>
                  </div>
                )}
              </div>
          ...
          >
            <div className="glass rounded-xl p-6 sm:p-8">
              ...
              <h3 className="text-xl sm:text-2xl font-semibold text-text-primary mb-4">No Shows Scheduled</h3>
        ...
        {pastGigs.length > 0 && (
          ...
          >
            <div className="flex items-center mb-8">
              ...
              <h3 className="text-2xl font-semibold text-text-primary">Past Shows</h3>
            </div>
            ...
            >
              <div className="flex gap-4 sm:gap-6 w-fit">
                ...
                {hasMorePast && !loadingMore && allPastGigs.length > pastGigs.length && (
                  <div className="w-80 sm:w-96 flex items-center justify-center">
                    <div className="glass rounded-xl p-6 text-center">
                      <div className="text-text-secondary mb-2">+{allPastGigs.length - pastGigs.length} more shows</div>
                      <div className="text-sm text-text-muted">Keep scrolling to load</div>
                    </div>
                  </div>
                )}
